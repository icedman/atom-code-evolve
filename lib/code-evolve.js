...
export default {
  ...
  executeCommands(commands) {
    ...
    let p = new Promise( (resolve, reject) => {
      ...
      if (c[0] == "line") {
        ...
        atom.workspace.open(c[1]).then( function() {
          ...
          setTimeout(function() {
            ...
            resolve(commands)
          }, self.fileSwitchDelay)

        })

      } else if (c[0] == "renameFrom") {

        console.log("renameFrom:" + c[1])
        self.lastCursorPosition = null
        atom.workspace.open(c[1]).then( function() {
          ...
          let editor
          if (editor = atom.workspace.getActiveTextEditor()) {
            self.selectedText = editor.getText()
          }
      ...
      } else if (c[0] == "renameTo") {

        console.log("renameTo:" + c[1])
        self.lastCursorPosition = null
        atom.workspace.open(c[1]).then( function() {

          setTimeout(function() {
            // paste clipboard
            atom.workspace.getActiveTextEditor().setText(self.selectedText)
            self.moveCursor({row:0,column:0})
    ...
    for(let i=0; i<diffs.length; i++) {
      ...
      for(let j=d; j<lines.length; j++) {
        ...
        if (l.startsWith("rename from")) {
          let file = l.replace("rename from ")
          commands.push([ 'renameFrom', file ])
        }

        if (l.startsWith("rename to")) {
          let file = l.replace("rename to ")
          commands.push([ 'renameTo', file ])
        }
